...
prompt.get(executiveOptions, function(err, res) {
    ...
    if (res.eOptions == 1) {
        ...
        console.log(color.bgWhite('You picked an invalid choice!'));
        connection.end();
    }
});
...
var createDepartment = function() {
    ...
    var newDepartment = {
        properties: {
            newDeptName: {
                description: colors.red('Please enter the name of the new department you would like to add.')
            },
            newOverhead: {
    ...
    prompt.get(newDepartment, function(err, res) {
        ...
        connection.query('INSERT INTO Departments (DepartmentName, OverHeadCosts, TotalSales) VALUES (?, ?, ?);', [newDept[0].deptName, newDept[0].overHeadNew, newDept[0].autoTotalSales], function(err, result) {
            if (err) {
                ...
                console.log('');
                console.log(colors.red.underline('New Department sucessfully created!'));
                console.log(' ');
                connection.end();
            }
        })
